...
- **`NUMPROCS` is unset inside a healthcheck.** `entrypoint.d/0200-set-cpu-count.sh` exports it into the entrypoint’s process tree; the daemon spawns each HEALTHCHECK probe as a fresh process, which inherits nothing. `tools.d/healthcheck.d` sources `detect-cpu-count` itself for this reason — any other parallel runner spawned outside the entrypoint tree must do the same, never trust an inherited `NUMPROCS`.
- **`test.d` doubles must not rely on executing files under `/tmp`.** That mount can be noexec, so a stub script there fails with `Permission denied` (exit 126) and the test silently exercises the real tool instead. Define the doubles as shell functions and `export -f` them into the child shell — see `test.d/0210-b19-fetch-near-cache.sh`.
...
- **`always/pre` runs on a bare base.** In the `foundation` stage it precedes `500-install-apt`, so nothing from `common.apt.deps` (`curl`, `aria2c`) exists yet; a tool it calls must degrade to Bash built-ins. A probe that cannot run exits `127`, which callers treat as fatal, never as “cache down”.
...
- **`update-apt` retries the whole `apt-get update`, not just apt’s own `Acquire::Retries`.** `archive.ubuntu.com`’s GeoDNS redirect lands on whatever official mirror is nearest (e.g. `ubuntu.mirrors.ovh.net` for OVH-hosted runners), and that mirror’s rsync is not atomic — a build can catch it mid-sync and get a transient `BADSIG` on `InRelease` that self-heals within seconds. `Acquire::Retries` does not reliably cover a signature-verification failure, so `update-apt` wraps the call in its own backoff loop instead of loosening `--error-on=any`. Known limitation: retries run per build inside the `flock`-serialized shared apt-lists cache, so a long bad-mirror window makes every queued build pay its own retry cost rather than the problem being solved once — a pinned/self-hosted mirror or an atomic snapshot service (e.g. `snapshot.ubuntu.com`) would remove this at the root; not yet done.
- **`--max-tries` does not cover most bad HTTP statuses.** aria2c 1.37 classifies only 503 (and network errors) as retryable; 500/502/504 all collapse into exit 22 and abort on the FIRST attempt, so `B19_DOWNLOAD_MAX_TRIES` never applies to them. Measured against a local server: 500 → 1 request, 503 → 4 requests, 404 → 1 request. The `--retry-on-*` options that would change this do not exist in 1.37. `b19-fetch` therefore wraps the whole download in its own `B19_DOWNLOAD_ATTEMPTS` loop with jittered backoff, retrying only `B19_DOWNLOAD_RETRY_CODES`; a 404 or checksum mismatch still fails on the spot. This matters most on GHA, where no near cache fronts `github.com` release downloads.
...
- **BuildKit hits the same symptom, GHA-only.** `docker buildx build` has its own analogous layer-differ gap, so any GHA-published b19 image (buildx is the GHA backend; Forgejo uses buildah) can ship the identical corrupted `${B19_HOME}` — measured directly on a published `b19/crystal` image (`ubuntu:ubuntu 755` instead of `ubuntu:root 775`). `M6E_BUILDX_HEAL` in `m6e/container/scripts/backends/buildx/buildx-build` and `projectfile/actions/container-build/buildx/build.sh` heals it the same way, as a plain `docker build` layer (never `docker buildx build`, which resolves `FROM` against whatever builder the caller has active via `buildx use` — invisible to an image only `docker load` put in the daemon).
